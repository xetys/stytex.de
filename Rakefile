...
task :watch do
  ...
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview","LC_ALL"=>"en_US.UTF-8"}, "jekyll build --watch")
...
task :preview do
  ...
  system "compass compile --css-dir #{source_dir}/stylesheets" unless File.exist?("#{source_dir}/stylesheets/screen.css")
  jekyllPid = Process.spawn({"OCTOPRESS_ENV"=>"preview", "LC"=>"en_US.UTF-8", "LC_ALL"=>"en_US.UTF-8"}, "jekyll build --watch")
  compassPid = Process.spawn("compass watch")
